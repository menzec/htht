...
def water_shed(image):
    ...
    open_binary = cv.morphologyEx(
        binary, cv.MORPH_OPEN, kernel, iterations=2)  #mophology binary,2次开操作
    ...
    cv.imshow('3-distance-t', (dist_norm * 50).astype(np.uint8))

    ret, surface = cv.threshold(dist, dist.max() * 0.65, 255, cv.THRESH_BINARY)
    cv.imshow('4-surface', surface.astype(np.uint8))
    ...
    print(ret)
    cv.imshow('6-markers', markers.astype(np.uint8))
...
src = cv.imread(
    r"D:/Code/HTHT/python/OpenCV-Python-Tutorial/data/water_coins.jpg")
